            ...
            <div class="col-lg-4 mt-4 mt-lg-0">
                <div>
                    ...
                    <div class="d-flex flex-wrap justify-content-between">
                        ...
                        <a target="_blank" href="<?= base_url('my-account/order-invoice/' . $order['id']) ?>" class='btn btn-primary'><?= label('download', 'download') ?></a>
                    </div>
                </div>
            </div>
        </div>
        <div class="row order-detail-box">
            <div class="col-lg-3">
                <?php foreach ($order['order_items'] as $key => $item) { ?>
                    <div class="row">
                        <div class="col-5 order-detail-img">
            ...
            if ($order['is_returnable'] && !$order['is_already_returned'] && isset($order_date) && !empty($order_date)) { ?>
                ...
                if ($today < $return_till && $item['type'] != 'digital_product') { ?>
                    <div class="col my-auto ">
                        <a class="update-order btn btn-danger mt-3 w-auto" data-status="returned" data-order-id="<?= $order['id'] ?>"><?= !empty($this->lang->line('return')) ? $this->lang->line('return') : 'Return' ?></a>
                    </div>
                <?php } ?>
            <?php } ?>
