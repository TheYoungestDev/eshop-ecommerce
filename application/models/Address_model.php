...
class Address_model extends CI_Model
{
    ...
    public function get_address_list($user_id = '')
    {
        ...
        foreach ($address_search_res as $row) {
            ...
            // print_R($theme);    
            if ($theme[0]['name'] == 'Modern') {    
                $btn = $row['is_default'] == 1 ? 'primary' : ' ';
                $class = $row['is_default'] == 1 ? '' : 'default-address ';
                $operate = '<div class="d-flex"><a href="javascript:void(0)" class="edit-address btn mr-1 mb-1" title="Edit" data-id="' . $row['id'] . '"  data-bs-toggle="modal" data-bs-target="#address-modal"><ion-icon name="create-outline" class="fs-4"></ion-icon></a>';
                $operate .= '<a href="javascript:void(0)" class="delete-address btn mr-1 mb-1" title="Delete" data-id="' . $row['id'] . '"><ion-icon name="trash-outline" class="fs-4"></ion-icon></a>';
                $operate .= '<a href="javascript:void(0)" class="' . $class . ' btn text-' . $btn . ' mr-1 mb-1" title="' . $default . '" data-id="' . $row['id'] . '"><ion-icon name="checkbox-outline" class="fs-4"></ion-icon></i></a></div>';
            } else {
